...
title: 腾讯极客技术挑战赛第三期
tags: [vite, 源代码]
summary: 在一个腾讯技术群里看到这个比赛，就顺手点开看了下，本以为会是算法比赛或者安全比赛。我尝试了一下之后，发现我也可以搞搞，因为是前端相关。我并不是专业搞安全的，也未能拿到很好的名次（69/927），不过在做题的过程中，我能突破自我，能想办法解决这些问题，还是很有成就感的。
---
...
![hmr-plugin-update-main-structure](/static/images/vite/hmr-plugin-update-main-structure.png)
...
function plantTree() {
  axios
    .get('http://159.75.70.9:8081/pull?u=0000023905914B302E216CE478979D59')
    .then(async function (response) {
      ...
      console.log(`t: ${t}, a: ${a}`)
      axios
        .get(`http://159.75.70.9:8081/push?t=${t}&a=${a}`)
...
window.A593C8B8 = async (_) =>
  (($, _, __, ___, ____) => {
    let _____ = (function* () {
      while ([])
        yield [(_, __) => _ + __, (_, __) => _ - __, (_, __) => _ * __][++__ % 3]['bind'](
          0,
          ___,
          ____
        )
    ...
    return new Promise((__) =>
      _['a']['forEach']((___) => $['setTimeout']((____) => ______(___, _____, __), ___))
    )
  })(window, _, +[], +[], +[])
```
...
![tencent-geeks-chanllenge-3-p4-code-structure](/static/images/tencent-geeks-chanllenge-3-p4-code-structure.png)
...
![tencent-geeks-chanllenge-3-p5-webassembly](/static/images/tencent-geeks-chanllenge-3-p5-webassembly.png)
