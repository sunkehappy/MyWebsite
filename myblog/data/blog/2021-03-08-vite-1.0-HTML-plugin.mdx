...
tags: [vite, 源代码]
summary: 通常我们开发的网站，第一次加载的就是HTML页面，vite会改写这个HTML，这是通过`src/node/server/index.ts`里面的`htmlRewritePlugin`来实现的。
---

通常我们开发的网站，第一次加载的就是HTML页面，vite会改写这个HTML，这是通过`src/node/server/index.ts`里面的`htmlRewritePlugin`来实现的。
代码截图：
![serverPluginHtml.ts](/static/images/vite/server-plugin-HTML.png)

我们可以看到，这个插件里面就一个`rewriteHTML`，别的都很简单，然后我们再看`rewriteHTML`里面的代码：
![serverPluginHtml.ts](/static/images/vite/call-rewrite-imports.png)
