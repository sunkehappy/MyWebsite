...
summary: 今天在用`List`的时候遇到一个问题，我的一个节目，需要用到嵌套的`List`，最开始的时候一直都搞不懂在渲染二级`cell`的时候，如何把数据传到渲染的函数中。因为我之前只有一层`List`的时候，都是在`class`中维护好了数据列表，假设叫`userArray`，然后在`list`的渲染函数`renderHandler`中，根据`index`来取数据的。
---
