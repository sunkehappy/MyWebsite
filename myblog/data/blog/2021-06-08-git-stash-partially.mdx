...
title: Git stash部分文件
tags: [git]
summary: 工作中`git stash`是一个很有用的命令，比如在当前分支上做开发，结果没做完，又要去另一个分支上修bug。有两种方式，一种是先临时提交下，后面再把这个提交修改掉。另一种就是先stash起来，后面再pop出来。当我们在做一些测试的时候，想比较两种修改，怎么样暂存一部分修改呢？
---
...
![git-stash-screenshort](/static/images/git-stash-screenshort.png)
